const unwrap = fn => event => fn('detail' in event ? event.detail : event.channel ?? event.data ?? event, event)
export const emit = (target, name, detail) => target.dispatchEvent(new CustomEvent(name, { detail, cancelable: true }))
