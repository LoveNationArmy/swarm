...
export default class Peer extends RTCPeerConnection {
  constructor (opts = {}) {
    ...
    on(this, 'icegatheringstatechange', () => {
      ...
      if (this.iceGatheringState === 'complete') {
        ...
        this.debug(this + ' send local description')
  ...
  async addMedia (media, fromOffer = false) {
    const stream = await navigator.mediaDevices.getUserMedia(media)
    this.media = media
    this.setLocalStream(stream)
    this.setLocalDescription(await this[fromOffer ? 'createAnswer' : 'createOffer']({ iceRestart: true }))
  }

  setLocalStream (stream) {
    this.debug(this + ' add local stream', stream.getTracks())
    if (this.localStream) {
